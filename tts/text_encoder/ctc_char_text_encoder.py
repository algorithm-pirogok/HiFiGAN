...
class CTCCharTextEncoder(CharTextEncoder):
    ...
    def __init__(
        ...
        path_to_lm: str = "data/lm/language_model.arpa",
    ):
    ...
    def ctc_beam_search(
        self, probs: torch.tensor, probs_length, beam_size: int = 1, **kwargs
    ) -> List[Hypothesis]:
        """
                ...
                for (pref, last_char), pref_proba in state_dict.items():
                    ...
                    if next_char == last_char:
                        ...
                        if next_char != self.EMPTY_TOK:
                            ...
                            new_pref = pref
                        last_char = next_char
                    new_state_dict[(new_pref, last_char)] += (
                        pref_proba * next_char_proba
                    )
        ...
        hypos: List[Hypothesis] = [
            Hypothesis(seq, prob) for (seq, _), prob in state.items()
        ]
        return sorted(hypos, key=lambda x: x.prob, reverse=True)

    def lm_batch_beam_search(
        self, probs, probs_length, multy_pool, size_of_beam_search=50, **kwargs
    ):
        batch = [
            prob[:length].detach().cpu().numpy()
            for prob, length in zip(probs, probs_length)
        ]
        return self.decoder.language_model.decode_batch(
            multy_pool, batch, size_of_beam_search
        )
