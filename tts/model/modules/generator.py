...
class Generator(nn.Module):
    def __init__(self, in_channels, upsample_channel, conv_kernels, conv_strides,
                 res_kernels, res_dilations):
        super().__init__()
        self.input_convolution = weight_norm(nn.Conv1d(in_channels=in_channels,
        ...
        self.resnet_blocks = nn.ModuleList(
            [nn.ModuleList([
                nn.LeakyReLU(),
                weight_norm(nn.ConvTranspose1d(in_channels=upsample_channel // (2 ** num),
                                               out_channels=upsample_channel // (2 ** (num + 1)),
                                               kernel_size=kernel,
                                               stride=stride,
                                               padding=(kernel - stride) // 2)),
                nn.ModuleList([
                    ResBlock(channels=upsample_channel // (2 ** (num + 1)),
                             kernel=resnet_kernel,
                             dilations=res_dilations) for resnet_kernel in res_kernels
                ])]
            )
        ...
        self.output_convolution = weight_norm(
            nn.Conv1d(in_channels=upsample_channel // (2 ** len(self.resnet_blocks)),
                      out_channels=1,
                      kernel_size=7,
                      stride=1,
                      padding=3))
