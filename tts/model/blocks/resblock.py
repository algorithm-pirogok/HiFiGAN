...
class ResBlock(nn.Module):
    def __init__(self, channels, kernel, dilations):
        ...
        self.first_convs = nn.ModuleList([
            weight_norm(nn.Conv1d(channels, channels, kernel, 1, dilation=dilations[0],
                                  padding=(kernel - 1) * dilations[0] // 2)),
            weight_norm(nn.Conv1d(channels, channels, kernel, 1, dilation=dilations[1],
                                  padding=(kernel - 1) * dilations[1] // 2)),
            weight_norm(nn.Conv1d(channels, channels, kernel, 1, dilation=dilations[2],
                                  padding=(kernel - 1) * dilations[2] // 2)),
        ])
        self.second_convs = nn.ModuleList([
            weight_norm(nn.Conv1d(channels, channels, kernel, 1, dilation=1, padding=(kernel - 1) // 2)),
            weight_norm(nn.Conv1d(channels, channels, kernel, 1, dilation=1, padding=(kernel - 1) // 2)),
            weight_norm(nn.Conv1d(channels, channels, kernel, 1, dilation=1, padding=(kernel - 1) // 2)),
        ])
